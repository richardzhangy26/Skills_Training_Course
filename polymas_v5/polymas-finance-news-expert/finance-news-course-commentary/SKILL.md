...
2. `[CALL] 平台通用工具公开网检索`：在 `course_evidence_available: true` 时，按课程主题和用户指定时间窗召回公开新闻，记录标题、URL、来源、发布时间、事实摘要和 `retrieved_at`；不使用付费墙内容或登录态。平台通用工具返回候选后，编排方依据 `source-policy.md` 声明 `source_tier`，但这只是声明；normalizer 必须用内部 hostname allowlist 独立推导等级，调用方不能提升来源等级，不得传入 `source_level`。若为 false，不生成点评候选，但仍将空候选数组交给 normalizer。
...
4. `[BUILD] normalize_candidates.py`：按数据契约对候选进行强制校验与排序。它使用内部 hostname allowlist 推导 `source_level`，要求声明的 `source_tier` 与推导结果一致，强制拒绝三级、未列入、私网或 userinfo 来源。它还校验课程身份、资源上限和所有将输出的字符串/键值，拒绝规避式投资建议。`rejected` 只包含 `candidate_index` 和 `reason`。
