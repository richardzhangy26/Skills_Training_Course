...
3. **选择运行模式**：启动后在终端选择 1~4 号模式：
   - `1. 半交互式（推荐）`：灵活的混合模式，支持手动和自动两种方式：
     - **直接回车**：让 Doubao 模型自动生成本轮回答（默认使用"优秀学生"档位）
     - **输入内容**：使用你输入的内容作为学生回答
     - **输入 `continue`**：进入全自动模式，后续所有轮次由 AI 自动回答
     - **输入 `continue N`**：自动运行到第 N 轮后暂停，恢复半交互模式（支持设置断点）
     - **输入 `quit`**：退出程序
     - 启动时可选择学生档位，也可预设断点轮数
   - `2. 自动化（预设答案）`：需要提前在脚本中修改 `user_answers` 列表。
   - `3. Doubao 自动回答`：脚本会提示你选择学生档位，可选是否提供**模拟对话 Markdown**与**外部知识库**，最终由 Doubao 生成回答。
   - `4. 日志回放`：指定之前生成的 `_dialogue.txt` 或 `_dialogue.json` 文件，脚本会根据 AI 的提问在日志中寻找最相似的问题并直接复用编辑后的回答，找不到时才回退到 Doubao。
...
```
第 1 轮：[直接回车] → AI 自动回答
第 2 轮：[输入"我认为是离心泵汽蚀"] → 使用手动回答
第 3 轮：[输入"continue"] → 后续全部自动运行到结束
```
...
结果：自动运行到第 5 轮后暂停，可以手动检查 AI 的提问质量
然后继续选择手动/自动/设置新断点
```

**场景 3：混合模式灵活切换**
```
第 1-3 轮：[直接回车] → 让 AI 快速通过基础问答
第 4 轮：[手动输入] → 在关键问题上给出特定答案
第 5 轮：[输入"continue 8"] → 自动到第 8 轮，然后再次人工介入
```

提示：
- 若需加载模拟对话或知识库，请输入绝对路径，脚本会自动读取内容并拼入提示词。
- Doubao 回答默认限制在 50 字内，且会自动识别确认式/选择式提问并直接输出"是""1""A"等短句。
- 半交互模式特别适合：探索新任务流程、调试特定环节、快速生成测试日志。
