...
1. **判定回应得当（接纳/倾听）**:
   ...
   - **操作**：**不要输出任何对话内容**。**仅输出**跳转关键词: `NEXT_TO_EXPLORE`
...
2. **判定回答正确（共情/情感反映）**:
   ...
   - **操作**：**不要输出任何对话内容**。**仅输出**跳转关键词: `NEXT_TO_CLOSURE`
...
- **操作**：**不要输出任何对话内容**。**仅输出**跳转关键词: `NEXT_TO_OUTLINE`
...
- **分支A（学生回答准确完整）**:
   ...
   - **操作**：**不要输出任何对话内容**。**仅输出**跳转关键词: `NEXT_TO_GOAL`
...
1. **判定学生共情到位**:
   ...
   - **操作**：**不要输出任何对话内容**。**仅输出**跳转关键词: `NEXT_TO_REFRAME`
...
- ✅ 严格的医学专业判定标准
- ✅ 跳转纯净性约束
- ✅ 状态追踪机制：Step 0 回溯确保累积检查的准确性

---

## 附录：跳转纯净性 - 正误对比示例

本节详细展示跳转纯净性的正确与错误写法，供编写训练剧本时参考。

### 核心原则

满足跳转条件时，**仅输出跳转关键词**，严禁任何其他字符。
...
```markdown
❌ 错误写法：
- **操作**：学生回答正确，回复"非常好！"，然后输出 `NEXT_TO_STAGE2`
...
- **分支 A（评价回答）**：
    - 条件：学生回答正确 (等类似意思)
    - 策略：肯定学生表现
    - 话术示例："非常好！你的思路很清晰。" (不要完全按照话术实例输出...)

- **分支 B（纯净跳转）**：
    - 条件：已评价且学生准备好进入下一阶段 (等类似意思)
...
```markdown
❌ 错误写法：
- **操作**：先输出评价语"回答正确"，再输出跳转关键词 `NEXT_TO_STAGE2`
...
|---------|-----------|---------|
| 先X再跳转 | "先...再..." | 拆分为两个独立分支 |
| 评价后跳转 | "评价...然后..." | 评价和跳转分不同分支处理 |
| 带前缀 | 跳转前有中文/英文 | 删除前缀，保留纯跳转关键词 |
| 带后缀 | 跳转后有中文/英文 | 删除后缀，保留纯跳转关键词 |
| 带标点 | 跳转后有标点符号 | 删除标点 |
| 带引号 | `"NEXT_TO_X"` | 移除引号 |
