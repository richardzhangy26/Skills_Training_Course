...
## training-background-generator (v1.0) 🆕

### 功能
根据训练剧本配置文档，为每个训练阶段自动生成16:9的写实中国风背景图片。

✨ **特色功能**:
- 🤖 LLM 智能理解阶段上下文，自动生成高质量文生图提示词
- 🎨 Doubao Seedream 文生图模型生成 512×288 (16:9) 背景图
- 📁 自动下载保存图片到本地 `backgrounds/` 目录
- 📝 自动回填图片路径到剧本 Markdown 的 `**背景图**` 字段
- 📊 生成 `generation_record.json` 记录文件

### 何时使用
当你需要为能力训练的各个阶段生成背景图片时使用此 skill:
- "生成阶段背景图"
- "为剧本生成图片"
- "生成训练背景"
- 或提供训练剧本配置文档并要求生成背景图
...
### 环境配置
需要在 `.claude/skills/.env` 中配置:
```bash
LLM_API_KEY=sk-xxx  # Polymas API Key（必需）
```

### 查看详细文档
- [SKILL.md](./training-background-generator/SKILL.md) - 完整技术文档
...
5. **training-background-generator** → 为每个阶段生成16:9背景图
...
└── 离心泵汽蚀故障紧急诊断/
    ...
    └── backgrounds/                  # training-background-generator生成
        ├── stage_1_xxx.png
        ├── stage_2_xxx.png
        └── generation_record.json
```
