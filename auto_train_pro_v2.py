...
直接阶段启动探针：设置 START_STEP_ID=<step.nid> 后，脚本仍先发送 scriptStart，
但在收到服务端首个 nextStep 时改发 stepStart{stepId: START_STEP_ID}，用于验证
trainV2 是否允许新会话从指定阶段启动。未设置时完全保持原协议流程。
"""
...
class TrainProClient:
    def __init__(self):
        ...
        # 可选的直接阶段启动协议探针。只替换服务端第一次 nextStep，避免影响后续正常流转。
        self.requested_start_step_id = os.getenv("START_STEP_ID", "").strip() or None
        self._start_step_probe_consumed = False
        self.start_step_probe_timeout = (
            float(os.getenv("START_STEP_PROBE_TIMEOUT", "60"))
            if self.requested_start_step_id
            else 60.0
        )
        self._start_step_progress = asyncio.Event()
        self._start_step_probe_task: Optional[asyncio.Task] = None
        self._start_step_probe_active = False
        self._start_step_probe_outcome: Optional[str] = None
        self._direct_jump_fallback_active = False
        self._start_step_fallback_sent = False
    ...
    def _select_step_to_start(self, server_step_id: Optional[str]):
        """首次 nextStep 可替换为探针目标；后续阶段始终服从服务端。"""
        if self.requested_start_step_id and not self._start_step_probe_consumed:
            self._start_step_probe_consumed = True
            selected_step_id = self.requested_start_step_id
            return selected_step_id, selected_step_id != server_step_id
        return server_step_id, False

    def _finish_start_step_probe(self, outcome: str, detail: str):
        """结束直接跳转探针；只有角色事件可标记为已推进，其它结果均明确区分。"""
        if not self._start_step_probe_active:
            return

        self._start_step_probe_active = False
        self._start_step_probe_outcome = outcome
        self._start_step_progress.set()

        if outcome == "progress":
            if self._start_step_fallback_sent:
                log.info(f"🧪 服务端在阶段开场应答后继续推进（{detail}）")
            else:
                log.info(f"🧪 目标 stepStart 后服务端已继续推进（{detail}），未补发阶段开场应答")
        elif outcome == "inconclusive":
            log.warning(f"🧪 直接跳转探针结果不确定：{detail}")
        else:
            log.info(f"🧪 直接跳转探针已终止：{detail}")

        task = self._start_step_probe_task
        current_task = asyncio.current_task()
        if task and not task.done() and task is not current_task:
            task.cancel()

    async def _cleanup_start_step_probe(self, reason: str):
        self._finish_start_step_probe("terminal", reason)
        task = self._start_step_probe_task
        if task and not task.done():
            task.cancel()
        if task and task is not asyncio.current_task():
            await asyncio.gather(task, return_exceptions=True)
        self._start_step_probe_task = None

    async def _wait_for_start_step_probe_exit(self):
        """等待已请求取消的探针任务复位阶段锁，避免后续 nextStep 漏掉开场处理。"""
        task = self._start_step_probe_task
        if task and task is not asyncio.current_task():
            await asyncio.gather(task, return_exceptions=True)
    ...
    async def disconnect(self):
        await self._cleanup_start_step_probe("会话断开")
        if self.ws:
            await self.ws.close()
        self.is_connected = False
        log.info("连接已断开")
    ...
    async def _handle_stage_entry(self):
        ...
        try:
            if self.auto_continue or self._direct_jump_fallback_active:
            ...
            for attempt in range(1, self.stage_entry_max_tries + 1):
                ...
                if self._direct_jump_fallback_active:
                    self._start_step_fallback_sent = True
                await self.send_user_text(text)
                await self.send_continue()
    ...
    async def _handle_direct_jump_fallback(self, target_step_id: str):
        """先观察服务端是否自然推进；超时后才补非首阶段开场应答，避免污染探针结论。"""
        try:
            await asyncio.wait_for(
                self._start_step_progress.wait(),
                timeout=self.start_step_probe_timeout,
            )
        except asyncio.TimeoutError:
            if (
                not self._start_step_probe_active
                or self.task_completed
                or self.step_id != target_step_id
            ):
                return
            log.warning(
                "🧪 目标 stepStart 后 %.1fs 无角色推进，开始尝试阶段开场应答兜底",
                self.start_step_probe_timeout,
            )
            self._direct_jump_fallback_active = True
            try:
                await self._handle_stage_entry()
            finally:
                self._direct_jump_fallback_active = False
    ...
    async def handle_message(self, message: str):
        ...
        if event == "connected":
            ...
            if self._start_step_probe_active:
                self._finish_start_step_probe(
                    "inconclusive",
                    "服务端在目标阶段产生角色事件前返回了新的 nextStep",
                )
                await self._wait_for_start_step_probe_exit()
            server_step_id = payload.get("nextStepId")
            selected_step_id, is_direct_jump = self._select_step_to_start(server_step_id)
            if not selected_step_id:
                log.error("❌ nextStep 未携带可用的 nextStepId")
                return

            self.step_id = selected_step_id
            self.step_index += 1
            if is_direct_jump:
                log.warning(
                    "🧪 直接跳转探针：服务端建议 %s，改发 stepStart: %s",
                    server_step_id,
                    self.step_id,
                )
            else:
            ...
            if is_direct_jump:
                self._start_step_probe_active = True
                self._start_step_probe_outcome = None
                self._start_step_fallback_sent = False
                self._start_step_progress.clear()
            ...
            if is_direct_jump:
                self._start_step_probe_task = asyncio.create_task(
                    self._handle_direct_jump_fallback(self.step_id)
                )
            elif self.step_index >= 2 and not self._stage_entry_running:
            ...
            self._finish_start_step_probe("progress", "selectRoleEnd")
            self.current_role = {
            ...
            self._finish_start_step_probe("progress", "botAnswerStart")
            ...
            self._finish_start_step_probe("terminal", "收到 scriptEnd")
            self.task_completed = True
            self._done.set()
            ...
            self._finish_start_step_probe("terminal", "服务端返回 error")
...
async def main():
    ...
    if client.requested_start_step_id:
        print(f"🧪 指定起始阶段: {client.requested_start_step_id}")
        print("   将在服务端首次 nextStep 后改发该 stepId；请观察是否进入目标阶段。")
